      ...
      if (linkage_technique == "P") {
        ...
        if("posterior_threshold" %in% names(acceptance_threshold)){
          ...
          if("log_scaled_plots" %in% names(extra_parameters) && extra_parameters[["log_scaled_plots"]] == T){
            decision_boundary <- ggplot(linkage_pairs, mapping = aes(mpost)) +
              geom_histogram(binwidth = 0.05, position = "identity", fill = "gray", colour = "black") +
          ...
          if("log_scaled_plots" %in% names(extra_parameters) && extra_parameters[["log_scaled_plots"]] == T){
            decision_boundary <- ggplot(linkage_pairs, mapping = aes(weight)) +
              geom_histogram(binwidth = 0.05, position = "identity", fill = "gray", colour = "black") +
              labs(x = "Match Weight", y = "Frequency") +
              geom_vline(aes(xintercept = acceptance_threshold, linetype = "Acceptance Threshold"),
                         color = "black", size = 0.6) +
    ...
    for(row_num in 1:nrow(iterations_df)){
      ...
      if(("extra_summary_parameters" %in% names(extra_parameters) && extra_parameters[["extra_summary_parameters"]] == TRUE)){
        if("performance_measure_variables" %in% names(results) && linkage_technique != "D"){
          ...
          # Print some of the performance metrics
          print(paste0('TP: ', TP))
          print(paste0('TN: ', TN))
          print(paste0('FP: ', FP))
          print(paste0('FN: ', FN))
          print(paste0('PPV: ', (TP/(TP + FP))))
          print(paste0('NPV: ', (TN/(TN + FN))))
          print('=============')
